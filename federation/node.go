...
// HTTP listener serving the federation protocol.
type Node struct {
	core  *core.Core
...
// [MeshPort] of the node's mesh address. The returned Node must be Stop()ed on
// shutdown. logger receives yggdrasil's info/warn/error output.
func Start(fc config.FederationConfig, logger *log.Logger) (*Node, error) {
...
// Stop shuts the mesh listener and the yggdrasil core down.
func (n *Node) Stop() {
...
// protocolHandler is the madnetwork protocol surface served on the mesh. F0
// ships only the ping; friendship/catalog/transfer endpoints mount here in
// later milestones.
...
func (c *coreLogger) Println(a ...interface{})          { c.l.Println(append([]interface{}{"federation:"}, a...)...) }
