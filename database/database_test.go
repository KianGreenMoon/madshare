...
func TestOpen_CreatesExpectedTables(t *testing.T) {
	...
	got := tableNames(t, db)
	want := []string{
		"access_group_members", "access_groups", "album_images", "api_tokens",
		"artist_images", "audit_log", "content_grants", "file_uploads", "files",
		"media_metadata", "role_permissions", "roles", "schema_migrations",
		"sessions", "settings", "user_roles", "users",
	}
	sort.Strings(got)
	if len(got) != len(want) {
...
func TestOpen_RecordsMigrationVersion(t *testing.T) {
	...
	if v != 8 {
		t.Errorf("migration version = %d, want 8", v)
	}
}
...
func TestOpen_IdempotentMigrations(t *testing.T) {
	...
	if rows != 8 {
		t.Errorf("schema_migrations row count = %d, want 8 after re-run", rows)
	}
}
