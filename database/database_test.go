...
func TestOpen_CreatesExpectedTables(t *testing.T) {
	...
	got := tableNames(t, db)
	want := []string{"file_uploads", "files", "media_metadata", "schema_migrations"}
	sort.Strings(got)
	if len(got) != len(want) {
...
func TestOpen_RecordsMigrationVersion(t *testing.T) {
	...
	if v != 1 {
		t.Errorf("migration version = %d, want 1", v)
	}
}
...
func TestOpen_IdempotentMigrations(t *testing.T) {
	...
	if rows != 1 {
		t.Errorf("schema_migrations row count = %d, want 1 after re-run", rows)
	}
}
