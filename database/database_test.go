...
func TestOpen_CreatesExpectedTables(t *testing.T) {
	...
	want := []string{
		...
		"data_sources", "federation_catalog",
		...
		"federation_mark_records", "federation_marks", "federation_nodes",
		"file_uploads", "files",
...
func TestOpen_RecordsMigrationVersion(t *testing.T) {
	...
	if v != 47 {
		t.Errorf("migration version = %d, want 47", v)
	}
}
...
func TestOpen_IdempotentMigrations(t *testing.T) {
	...
	if rows != 47 {
		t.Errorf("schema_migrations row count = %d, want 47 after re-run", rows)
	}
}
