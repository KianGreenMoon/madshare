...
func TestOpen_CreatesExpectedTables(t *testing.T) {
	...
	want := []string{
		...
		"data_sources", "federation_peers", "file_uploads", "files",
...
func TestOpen_RecordsMigrationVersion(t *testing.T) {
	...
	if v != 26 {
		t.Errorf("migration version = %d, want 26", v)
	}
}
...
func TestOpen_IdempotentMigrations(t *testing.T) {
	...
	if rows != 26 {
		t.Errorf("schema_migrations row count = %d, want 26 after re-run", rows)
	}
}
