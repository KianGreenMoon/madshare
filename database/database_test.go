...
func TestOpen_CreatesExpectedTables(t *testing.T) {
	...
	want := []string{
		...
		"data_sources", "federation_catalog", "federation_claim_reports",
...
func TestOpen_RecordsMigrationVersion(t *testing.T) {
	...
	if v != 35 {
		t.Errorf("migration version = %d, want 35", v)
	}
}
...
func TestOpen_IdempotentMigrations(t *testing.T) {
	...
	if rows != 35 {
		t.Errorf("schema_migrations row count = %d, want 35 after re-run", rows)
	}
}
