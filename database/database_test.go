...
func TestOpen_CreatesExpectedTables(t *testing.T) {
	...
	want := []string{
		...
		"federation_holdings", "federation_mark_records", "federation_marks", "federation_peers",
		"file_uploads", "files",
		"image_processing_jobs", "library_upgrades", "madnetwork_cache", "media_analysis_jobs",
		"media_metadata",
...
func TestOpen_RecordsMigrationVersion(t *testing.T) {
	...
	if v != 42 {
		t.Errorf("migration version = %d, want 42", v)
	}
}
...
func TestOpen_IdempotentMigrations(t *testing.T) {
	...
	if rows != 42 {
		t.Errorf("schema_migrations row count = %d, want 42 after re-run", rows)
	}
}
