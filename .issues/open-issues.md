...
| Info | **`TestChaosSeederVanishesMidTransfer` is capacity-marginal under `-race`.** Two distinct flakes surfaced in the 2026-08-24 full-suite race run. (1) The assertion demanded a *failover*, but since F9 item 4 a vanished seeder's in-flight chunks are legitimately rescued by a **hedge winning the race** before the failure lands (`failovers=0 hedges=2/2`, transfer perfect) — **FIXED 2026-08-24**: the pin now accepts failover OR hedge-won, both being re-routes (repro: 2 of 8 race runs). (2) The 4 MiB fixture is marginal against the race-clocked netstack's ~50 KiB/s CPU-bound loopback ceiling: 1 of 4 stress runs degraded to whole mode and hit the transfer deadline with 3.7 of 4 MiB delivered — the same race-throughput artifact that makes the balance tests gate their split assertions on `testTimeoutScale == 1`. Not fixed: shrinking the fixture under race would change what the test exercises; recorded so a lone race failure of this test is read as environment, not regression. | open |
...
## Test — `TestStaleHoldersCostAFetch` is red at HEAD (found 2026-08-14) — **FIXED**
