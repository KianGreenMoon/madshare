// Full Library · Recordings — the recording-centric curation lens (recording-
// tagsets P5, docs/architecture/recording-tagsets.md "Admin surfaces"), a
// sub-mode of the Library page's Full Library scope (mounted by files.js).
// Every recording, newest first, windowed (virtual-list.js in window-scroll
// mode) with search + filter pills. A card expands to both arms stacked:
// renditions (ladder-ranked, live + soft-removed; Play / Split off / Remove /
// Restore) over appearances (primary marked, review state; Edit / Set primary
// / Move… / Remove), plus the whole-recording footer (Trash = soft; permanent
// delete lives on the Trash page). Merge is selection-based: tick ≥2
// recordings, the bulk bar wakes, the modal picks the surviving target
// (default = the ladder-best holder).
//
// Bespoke module (like moderation.js/mine-list.js): the two-arm expanded card
// is a shape file-list.js's flat rows can't express. Reuses the page's shared
// preview player (injected as `play`), the track-edit modal, the toast system,
// and the virtual-list windowing.
//
// Gates: the lens is offered to content.moderate holders (files.js omits it
// otherwise); delete/trash/remove need file.delete, access edit + tag edit
// need metadata.edit — gated client-side by hiding controls (the API enforces
// for real).
...
export function createRecordingsView({ play, perms }) {
  const panel     = document.getElementById('libMode-recordings');
  ...
  const canModerate = perms.includes('content.moderate'); // gates Add appearance, like Move/Merge
  ...
  // ── Preview (via the page's shared player) ──────────────────────────────────
  ...
  function highlightRow(key) {
    ...
    if (key != null) document.querySelector(`tr[data-file-id="${key}"]`)?.classList.add('rec-row--playing');
  }
  ...
  // ── Merge modal (ids prefixed rec- — the entity merge modal owns mergeModal) ─
  const mergeModal = document.getElementById('recMergeModal');
  ...
  function wire() {
    ...
    const hideMerge = wireModal(mergeModal,
      [document.getElementById('recMergeClose'), document.getElementById('recMergeCancel')]);
    ...
    // Filters / search.
    panel.querySelectorAll('.rec-pill').forEach(p => p.addEventListener('click', () => {
      panel.querySelectorAll('.rec-pill').forEach(x => x.classList.remove('rec-pill--on'));
    ...
    // Add appearance is content.moderate — this lens's own gate, matching Move /
  ...
  // openRecording deep-links one recording: search for #id and expand it (the
  // recording links on the file lenses / Trash lenses land here).
  async function openRecording(id) {
    if (!mounted) { mounted = true; wire(); }
    searchEl.value = `#${id}`;
    query = `#${id}`;
    await reload();
    const rec = vlist.getItems().find(r => r.id === id);
    if (rec && !expanded.has(rec.id)) toggleExpand(rec);
  }

  return {
    mount() { if (mounted) return; mounted = true; wire(); reload(); },
    reload() { if (mounted) reload(); },
    openRecording,
  };
}
