...
| `POST /api/my/uploads/bulk` | Batch `{"action": "submit"\|"remove", "tagset_ids": [...] \| "filter": {q,field}, "all": bool}` over the owner's `draft`+`returned` set (`all:true` required when the filter term is blank). `submit` shares the `/submit` semantics; `remove` discards to Trash. Backs "Select all N matching". |
